...
function checkRiceCooker (id: number): boolean | undefined {
  ...
  if (target != null) {
    ...
    if (!target.isCooking && target.isOperational && target.isPlugged) {
      ...
      if (target.isCooking) errorMessage += `Rice cooker id:${id} is still cooking.`
      if (!target.isOperational) errorMessage += `Rice cooker id:${id} is not operational.`
      if (!target.isPlugged) errorMessage += `Rice cooker id:${id} is not plugged in. `
...
function cookRice (): void {
  ...
  const riceCupsNb: number = +readline.question('Enter the number of cups of rice (should not be more than half of the number of cups of water): ')

  if (rcId == null || isNaN(rcId) || waterCupsNb == null || isNaN(waterCupsNb) || riceCupsNb == null || isNaN(riceCupsNb)) {
...
function boilWater (): void {
  ...
  if (isNaN(rcId) || isNaN(waterCupsNb) || rcId <= 0 || waterCupsNb <= 0) {
    console.log('Invalid values, please check your entries')
    boilWater()
  } else {
    if (checkRiceCooker(rcId)) {
      startCooking(rcId)
